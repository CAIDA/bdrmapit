...
from utils.utils import ls, stop_cluster, create_cluster, setup_parallel
        ...
        for j in f:
            if j['type'] == 'trace':
                ...
                if original_hopslist:
                    ...
                    if adjacencies_output or dest_pairs_output:
...
def parallel_init(addresses, adjacencies, destpairs, kargs, rt_obj=None, rir_obj=None):
    global ip2as
    global addresses_output, adjacencies_output, dest_pairs_output
    if rt_obj is None:
...
def setup_cluster(poolsize):
    stop_cluster()
    create_cluster(poolsize)
...
def main():
    ...
    if args.poolsize >= 0:
        pool = multiprocessing.Pool(args.poolsize, parallel_init, (args.addresses, args.adjacencies, args.destpairs, kargs))
    else:
        parallel_init(args.addresses, args.adjacencies, args.destpairs, kargs)
        pool = None
    parse_files(files, pool)
    if pool:
        pool.close()
    # addresses, adjacencies, destpairs = parse_files(files, pool)
    # if args.addresses:
    #     log.info('Writing addresses')
    #     write_addresses(args.addresses, addresses)
    # if args.adjacencies:
    #     log.info('Writing adjacencies')
    #     write_adjacencies(args.adjacencies, adjacencies)
    # if args.destpairs:
    #     log.info('Writing dest pairs')
    #     write_destpairs(args.destpairs, destpairs)
