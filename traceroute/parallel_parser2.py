                ...
                if original_hopslist:
                    ...
                    if hopslist:
                        ...
                        while i < numhops - 1:
                            ...
                            if hop2['asn'] > -2:
                                dest_pairs.add((addr2, dest_asn, issuspicious, loop))
                            if addr1 != addr2:  # Skip links with the same address
                                ...
                                if qttl2 == 0:
                                    if not (hop3 and (addr2 == hop3['addr'] or hop2['reply_ttl'] - hop3['reply_ttl'] == (hop3['probe_ttl'] - hop2['probe_ttl']) - 1)):
                                        qttl2 = 1
                                    # if hop3 and addr2 == hop3['addr']:
                                    #     pass
                                    #     # r['Addr2 == Addr3'] += 1
                                    # elif hop3 and hop2['reply_ttl'] - hop3['reply_ttl'] == (hop3['probe_ttl'] - hop2['probe_ttl']) - 1:
                                    #     pass
                                    #     # r['rTTL dist == pTTL dist - 1'] += 1
                                    # else:
                                    #     qttl2 = 1
                                    #     # r['Unknown 0'] += 1
                                elif qttl2 > 1:
                                    if not (hop2['icmp_type'] == 3 and hop2['icmp_q_ttl'] - hop1['icmp_q_ttl'] >= hop2['probe_ttl'] - hop1['probe_ttl']):
                                        qttl1 = 1
                                        qttl2 = 1
                                    # if hop2['icmp_type'] == 3 and hop2['icmp_q_ttl'] - hop1['icmp_q_ttl'] >= hop2['probe_ttl'] - hop1['probe_ttl']:
                                    #     pass
                                    #     # unknown.append(hopslist)
                                    # else:
                                    #     qttl1 = 1
                                    #     qttl2 = 1
                                    #     # r['Unknown > 1'] += 1
...
def parallel_init(addrq, adjq, dpq, distq, testq, kargs):
    global ip2as, addrqueue, adjqueue, dpqueue, distqueue, testqueue
...
def combine_addresses(queue, filename):
    ...
    while True:
        newaddrs = queue.get()
        if newaddrs == 'DONE':
            # print('Writing Addresses {:,d}'.format(len(addresses)))
            write_addresses(filename, addresses)
            break
        addresses.update(newaddrs)
...
def combine_adjacencies(queue, filename):
    ...
    while True:
        newadj = queue.get()
        if newadj is None:
            # print('Writing Adjs {:,d}'.format(len(adjacencies)))
...
def combine_destpairs(queue, filename):
    ...
    while True:
        newdp = queue.get()
        if newdp is None:
            # print('Writing DPs {:,d}'.format(len(destpairs)))
...
def combine_distances(queue, filename):
    ...
    while True:
        newdists = queue.get()
        if newdists is None:
            # print('Writing Dists {:,d}'.format(len(distances)))
