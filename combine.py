...
from traceroute.parser_pool_sqlite import opendb
...
def combinedbs(files, output):
    ...
    for filename in pb.iterator(files):
        ...
        adjs.update(cur.execute('SELECT * FROM adjacency'))
        dps.update(cur.execute('SELECT * FROM destpair'))
        dists.update({(h1, h2): num for h1, h2, num in cur.execute('SELECT * FROM distance')})
        cur.close()
        con.close()
