...
def graph_refinement(bdrmapit: Bdrmapit, routers: List[Router], interfaces: List[Interface], iterations: int = -1,
                     previous_updates: List[Tuple[Updates, Updates]] = None) -> Tuple[Updates, Updates]:
    rupdates = Updates()
    iupdates = Updates()
    iteration = 0
...
def reallocated_test(bdrmapit: Bdrmapit, oasn, newasn):
    ...
    log.debug(
        'Reallocated Test: conesize={} < 5 and conesize={} < oldcone={} and not peer_rel({}, {}) = {}'.format(
            conesize, conesize, bdrmapit.bgp.conesize[oasn], newasn, oasn, bdrmapit.bgp.peer_rel(newasn, oasn)))
